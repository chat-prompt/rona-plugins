...
방향 카드를 `submit_progress` 로 보낸 **직후 같은 턴에**, 셸(Bash)을 쓸 수 있으면 진행표를 브라우저로 **자동으로 한 번만** 연다(매 단계 반복이 아니라 이 최초 1회만). progress-live 링크의 `<tracking_token>` 자리에는 frontmatter `metadata.tracking_token` 값을 채운다(본문에 평문으로 늘어놓지 않는다).
...
명령이 정상 종료했는지만 조용히 확인하고, 사용자에게 진행표가 열렸는지 따로 묻지 않는다. 셸이 없는 커넥터(Cowork 등)만 링크를 안내한다.
