...
방향 카드를 `submit_progress` 로 보낸 **직후 같은 턴에 반드시**, 셸(Bash)을 쓸 수 있으면 진행표를 브라우저로 **자동으로 한 번만** 연다(매 단계 반복이 아니라 이 최초 1회만). progress-live 링크의 `<tracking_token>` 자리에는 frontmatter `metadata.tracking_token` 값을 채운다(본문에 평문으로 늘어놓지 않는다).

- macOS: `open "https://rona.so/skill/api/install/<tracking_token>?type=progress-live"`
- Linux: `xdg-open "https://rona.so/skill/api/install/<tracking_token>?type=progress-live"`
- Windows: `start "" "https://rona.so/skill/api/install/<tracking_token>?type=progress-live"`

명령이 정상 종료했는지만 조용히 확인하고, 사용자에게 진행표가 열렸는지 따로 묻지 않는다. **진행표를 열지 않은 채로 다음 단계로 가지 않는다.** 셸이 없는 커넥터(Cowork 등)만 링크를 안내한다.

진행 현황을 보여줄 때는 `get_progress` MCP tool 로 지금까지의 단계를 확인해 사람 말로 전하고, 시각 진행표가 필요하면 progress-live 링크(`https://rona.so/skill/api/install/<install_token>?type=progress-live`, `<install_token>` 자리에 frontmatter `metadata.tracking_token` 값을 채운다)를 전한다. 셸을 쓸 수 있으면 위 open 명령으로 바로 열고, 셸이 없는 커넥터만 "이 링크를 눌러 진행표를 여세요" 라고 안내한다(토큰을 본문에 평문으로 늘어놓지 않는다).
