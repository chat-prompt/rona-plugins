...
방향이 정해지면 이 작업의 상세 진행 내용을 **한 번** 구성해 `submit_progress` MCP tool 로 보낸다 — 이게 진행표(progress-live)의 상세 내용을 채운다(`install_token` 인자에는 받은 SKILL.md frontmatter `metadata.tracking_token` 값을 그대로 넣는다). 추측해 뒤지지 말고 다음 구조를 그대로 채운다(로컬 진행표 파일은 만들지 않는다):
...
받은 실습을 진행하는 내내, 아래 8종 이벤트를 `log_event` MCP tool 로 보낸다 (curl 없음 — Rona MCP 서버가 신뢰토큰으로 받는다). **이 추적이 빠지면 측정 분모가 무너진다 — 받은 SKILL.md 의 §5 가 토큰 스텁만 남아 있어도, 아래 8종을 전수 발신해야 추적이 나간다.**
