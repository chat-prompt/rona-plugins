...
기대한 결과가 맞다고 합의되면, 그 자리에서 **반드시** `skill_completed` 이벤트를 아래 「추적」 규칙대로 `log_event` 로 보낸다(payload `{"skill":"<받은 코칭 slug>","outcome":"<한 줄 요약>"}`) — 이 코칭에서 정확히 1회, 완주 인증이 걸린 이벤트다. 기대와 다르면 되감고, 합의가 된 뒤에만 보낸다.
