...
const pluginReviewMaskUrl = new URL('../plugins/rona-coach/skills/rona-coach/scripts/mask-review.mjs', import.meta.url);
...
const supportReviewMaskTarget = process.env.RONA_SUPPORT_SKILL
  ? resolve(dirname(process.env.RONA_SUPPORT_SKILL), 'scripts', 'mask-review.mjs')
  : new URL('../../rona-support/assets/rona-coach/scripts/mask-review.mjs', import.meta.url);
...
const [plugin, support, pluginSync, supportSync, pluginReviewMask, supportReviewMask, pluginManifest, supportManifest, pluginReview, supportReview, pluginReviewSkill, supportReviewSkill] = await Promise.all([
  readFile(pluginUrl, 'utf8'),
  readFile(supportTarget, 'utf8'),
  readFile(pluginSyncUrl, 'utf8'),
  readFile(supportSyncTarget, 'utf8'),
  readFile(pluginReviewMaskUrl, 'utf8'),
  readFile(supportReviewMaskTarget, 'utf8'),
...
if (missing.length) {
  ...
  process.exitCode = 1;
} else if (plugin !== support || pluginSync !== supportSync || pluginReviewMask !== supportReviewMask || pluginManifest !== supportManifest || pluginReview !== supportReview || pluginReviewSkill !== supportReviewSkill) {
